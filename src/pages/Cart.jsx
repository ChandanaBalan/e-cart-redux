        ...
        <div className='md:grid grid-cols-[2fr_1fr] m-10'>
              ...
              <tbody>
                {cartArray?.map((item,index) => (
                  <tr>
                    ...
                    <td className='border border-gray-200 text-center'>{item.title}</td>
                    ...
                    <td className='border border-gray-200 text-center'>
                      <button onClick={()=>dispatch(removeCartitem(item?.id))} className='bg-red-700 px-5 py-3 rounded text-white hover:border hover:bg-transparent hover:border-red-700 hover:text-red-700'><FontAwesomeIcon icon={faTrash} /></button></td>
                  </tr>
                ))
                }
              </tbody>
          ...
          <div className='pt-5 px-10'>
            <div className='shadow-lg p-5'>
              ...
              <p className='mt-5 text-xl'>Total number of products : <span>{cartArray.length}</span></p>
              <p className='mt-3 text-xl'>Grand Total : <span>$ {total}</span></p>
